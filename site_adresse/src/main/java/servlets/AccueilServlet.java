...
public class AccueilServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        //Sélection des listes de même type
        while(it.hasNext() && i<9) {
            Article article = it.next();
            if (article.getType().equalsIgnoreCase("Bar") && listBar.size()<3) {
                listBar.add(article);
                i++;
            }else{
                if (article.getType().equalsIgnoreCase("Restaurant") && listRestaurant.size()<3){
                    listRestaurant.add(article);
                    i++;
                }else{
                    if(article.getType().equalsIgnoreCase("Musee") && listMusee.size()<3){
        ...
        //Affichage dynamique des etoiles representant la note
